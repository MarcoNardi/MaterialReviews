...
% --------------------------------------------------------------------------------------
	...
	L'ambiente del Material Design  \cite{environment} specifica come sono costruiti e come si comportano gli elementi dell'interfaccia utente.
		...
		Le superfici si comportano come dei solidi, che possono cambiare forma e opacità, possono unirsi e dividersi, spostarsi e ruotare. Tutti gli elementi della UI del Material Design sono il risultato delle modifiche degli attributi di una superficie di base, che è bianca opaca, di spessore \texttt{1dp}, con un'ombra.
	...
	\section{Layout}
		...
		\subsection{Responsive layout grid}
			...
			\begin{itemize}
				...
				\item \textit{Grondaie}: definisco lo spazio che separa le colonne
		...
		\subsection{Densità}
			...
			Se l'utente deve interagire con molte informazioni, si possono rendere le informazioni più compatte diminuendo lo spazio tra loro, rendendole più facili da consultare e confrontare.

			È invece sconsigliato usare una alta densità di contenuti nei componenti che si concentrano nello svolgere una singola azione (\textit{focused task}) o nei messaggi d'errore
...
% --------------------------------------------------------------------------------------	
	\section{Navigazione}
    	...
    	Solitamente esiste una gerarchia tra le schermate di un'applicazione. Questa gerarchia si riflette nell'interfaccia che permette all'utente di navigare.
   ...
   \section{Suono}
        ...
        \subsubsection{Quando non usare il suono}
              ...
              \subsubsection{Tipi di suono}
				\begin{itemize}
					\item \textbf{Hero sound}: si verifica in interazioni fondamentali, evocando uno stato emotivo o celebrando un certo tipo di eventi;
...
% --------------------------------------------------------------------------------------
	...
	Se una forma non è interattiva, è necessario evitare l'utilizzo di forme con dimensioni sufficientemente grandi da apparire interattive.
...
% --------------------------------------------------------------------------------------		
	...
	\section{Motion}
		...
		\subsection{Transition patterns} 
			Material design fornisce dei patterns per transitions e sono:
			\begin{itemize}
		...
		\subsubsection{Container transform}
			...
			il container transform è molto utile per mostrare relazioni tra elementi, come per esempio una scheda e i suoi dettagli.
...
\chapter{Novità del Material Design 3}
	\section{Accessibilità}
		...
		\subsection{Touch targets}
			I touch targets sono parti dello schermo che rispondono ad eventi di tocco, vengono consigliate delle misure adatte a renderli facili da premere. Nella nostra applicazione i touch targets automaticamente implementati da Jetpack Compose, come switches o IconButtons, sono già creati con una dimensione adeguata,
			mentre i touch targets custom li rendiamo conformi allo standard invocando le funzioni della classe \texttt{Modifier}
		...
		\subsection{Testo}
			Uno dei modi migliori per aumentare l'accessibilità è l'aggiunta di didascalie, etichette e testo alternativo, in quanto sono facilmente leggibili dai programmi di lettura dello schermo, i quali possono avere delle difficoltà a "leggere" un'immagine.
			
			Il testo alternativo aiuta a tradurre e descrivere all'utente un elemento visivo, come un'immagine, in un elemento testuale.
		\subsection{Contrasto}
			Per rendere gli elementi più leggibilii all'interno dell'applicazione è necessario che i caratteri e le icone abbiano un contrasto sufficientemente elevato rispetto allo sofndo. Per il testo grande (titoli, sottotitoli, ...) è consigliato di avere un rapporto minimo di 3:1, mentre deve essere almeno 4.5:1 per il testo piccolo.

		\subsection{Best practice}
			La pratica migliore consiste nell'utilizzo di elementi nativi (nel nostro caso di Jetpack Compose), in quanto è garantito dagli autori della libreria/framework un adeguato livello di accessibilità.
			
			In ogni caso è possibile definire o modificare i valori di colore, dimensione, forma degli elementi di UI, ma questi devono essere testati adeguatamente per assicurarsi che rispettino le linee guida di accesibilità.
	...
	Un livello può essere applicato a un intero elemento o in una forma circolare e può essere applicato solo ad un livello alla volta.
	...
	Una delle principali novità introdotte dal Material Design 3 è il nuovo sistema dei colori, in particolare il sistema di colori \textit{dinamici} (disponibile solo in Android 12).
		...
		\begin{itemize}
			...
			\item \textit{colori on[...]}: garantiscono un contrasto adeguato tra il testo e il colore della superficie a cui si riferiscono
		\end{itemize}
		...
		Il colore primario deve essere utilizzato per gli elementi più importanti, come pulsanti, o per indiacare che un certo elemento è selezionato. In altre parole, il colore primario viene utilizzato per attirare l'attenzione dell'utente e suggerirgli le azioni da compiere.
		
		Il colore secondario viene utilizzato sulle componenti di importanza minore, come la barra di navigazione e altri elementi per svolgere azioni di bassa priorità.
		...
		\begin{figure}[h]
   			\centering
   			\includegraphics[width=0.4\textwidth]{colori_default} %nome del file
   			\includegraphics[width=0.4\textwidth]{colori_dinamici} %nome del file
...
\cleardoublepage
	...
	Ciò consente di conferrire maggior espressività all'interfaccia utente. Viene, inoltre, mantenuta la possibilità avere forme degli angoli simmetriche o asimmetriche, con angoli tagliati o arrotondati.
	...
	\section{Iconografia}
		...
		Sono state aggiunte delle linee guida nell'utilizzo delle icone, e nelle caratteristiche da usare nella personalizzazione: per esempio nella decisione del weight (spessore dell'icona) è bene che non sia troppo fino (sconsigliati valori sotto al 200), e che non sia troppo grosso; soprattutto bisogna stare attenti ad usare lo stesso spessore in modo consistente e non mischiare icone di weight diversi

		Una caratteristica molto importante è la grandezza ottica, che ci permette di cambiare la grandezza di un'icona da 24dp fino a 48dp aggiustando però il weight; infatti, scalando un'icona scalerebbe anche il suo spessore il che può non essere desiderato a seconda del proprio stile.
		Per quanto riguarda l'accessibilità è importante che icone che eseguono delle azioni abbiano un'etichetta che descriva cosa facciano, nella nostra applicazione le icone che eseguono azioni come scrivere una recensione, oppure cambiare tab, hanno tutte un testo che descrive il loro ruolo.
	\section{Tipografia}
		Nel Material Design 3 sono state modificate radicalmente le possibili dimensioni del testo. Sono state definite 5 nuove categorie:
		\begin{itemize}
			\item \textit{display}: per testi brevi e importanti, di dimensioni molto grandi
			\item \textit{headline}: per testi brevi e importanti in schermi piccoli
			\item \textit{title}:	per testi che sono mediamente importanti ma comunque brevi
			\item \textit{body}: per i testi più lunghi
			\item \textit{label}: per il testo che accompagna componenti come pulsanti o didascalie
		\end{itemize}
		
		Ciò è stato fatto per rendere più semplice applicare le linee guida e rendere i ruoli più facili da capire.
		A questo scopo sono stati inoltre definiti dei \textit{tokens} per il testo, per permettere una facile modifica di valori come dimensione e peso.
		Sono comunque validi tutti i principi illustrati nel Material Design 2.
...
\chapter{Material Reviews}
	...
	L'idea è nata dalla necessità di creare un'applicazione sufficientemente complessa da poter mettere in pratica più principi del Material Design possibili.
	
	Abbiamo deciso di utilizzare le libreire Jetpack Compose \cite{compose} e Room\cite{room} per l'implementazione della nostra app, rispettivamente per l'aspetto grafico e per la realizzazione del database.
	...
	L'applicazione è divisa in 3 parti principali: i ristoranti, le proprie recensioni e il proprio profilo.




	\subsection{Ristoranti}
	...
	\begin{figure}[h]
		\centering
		\includegraphics[width=0.3\textwidth]{screenHome.jpg} 
	  \caption{Lista dei ristoranti}
		
	\end{figure}
	
	Cliccando sulla card di un ristorante si apre la seguente schermata, che contiene dettagli e recensioni:
	\begin{figure}[h]
		\centering
		\includegraphics[width=0.3\textwidth]{screenDetails.jpg} 
	  \caption{Dettagli di un ristorante}
		
	\end{figure}


	Cliccando sul Floating Action Button per aggiungere la recensione si apre comparire un dialog dove si può scegliere una valutazione da dare e scrivere una recensione, come mostrato in Figura 4.3
	\pagebreak
	...
	Nella parte recensioni è possibile vedere le proprie recensioni. 
	
	Cliccare sulla recensione porta alla schermata di dettagli del ristorante stesso a cui si riferisce.
	Si possono anche rimuovere le recensioni con il pulsante dedicato, che fa apparire un dialog di conferma.
	...
	\newpage
	\subsection{Profilo}
		Nella sezione dedicata al profilo l'utente può modificare le proprie informazioni e le impostazioni del tema.
